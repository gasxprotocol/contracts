...
/**
 * @title GasX signed-policy fork integration on Base Sepolia (the A1 end-to-end bridge, second live chain)
 * @notice Mirror of the proven Arbitrum Sepolia probe, run against Base Sepolia (forked) on the SAME canonical
 *         v0.9 EntryPoint 0x433709009B8330FDa32311DF1C2AFA402eD8D009. A real EIP-712 SignedApproval from a
 ...
 * @dev Self-forks via vm.createSelectFork(base_sepolia); set BASE_SEPOLIA_RPC_URL to override the RPC.
 *      Gated by setUp: if Base Sepolia ever lacks the canonical v0.9 EntryPoint, every test self-skips.
 *      Verified 2026-06-15: `cast code 0x4337...D009 --rpc-url https://sepolia.base.org` returns 44852 bytes
 *      of bytecode, so the canonical v0.9 EntryPoint is present and these tests run for real.
 */
contract GasXSignedPolicyBaseForkTest is Test {
    ...
    bytes32 internal constant C = keccak256("campaign.base.fork");

    function setUp() public {
        // Env override BASE_SEPOLIA_RPC_URL wins; otherwise fall back to the foundry.toml base_sepolia endpoint.
        string memory rpc = vm.envOr("BASE_SEPOLIA_RPC_URL", vm.rpcUrl("base_sepolia"));
        vm.createSelectFork(rpc);
        // GATE: Base Sepolia must host the canonical v0.9 EntryPoint, exactly as Arbitrum Sepolia does.
        // If it is ever absent, skip the whole probe rather than fake it (Arbitrum remains the live proof).
        if (ENTRYPOINT_V09.code.length == 0) {
            emit log("Base Sepolia lacks the canonical v0.9 EntryPoint (0x4337...D009); probe gated until deployment.");
            vm.skip(true);
            return;
        }
